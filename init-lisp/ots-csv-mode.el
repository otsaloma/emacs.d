    ...
    (save-excursion
      (let (count count-header (csv t))
        ...
        (while (not (eobp))
          ...
          ;; If separator found and the amount if fields matches
          ;; that of the header line, continue to the next line.
...
(defun ots-csv-mode-set-properties ()
  ...
  (setq-local csv-align-padding 2)
  (setq-local csv-align-style 'right)
